...
This project used this [template](https://github.com/joshSi/cmake-sfml-project/). The readme of the template when it was cloned can be seen below after the separator.
...
sudo apt install \
    ...
    libegl1-mesa-dev
```
3. `cd` into the root of this repos then run :
```
cmake -S . -B build -DCMAKE_BUILD_TYPE=Release
cmake --build build
```
4. Copy the fond in the newly created `build/bin` folder then cd into it and run the programe :
```
# from the root of the repo
cp src/Aileron-Regular.otf build/bin
cd build/bin
./DotNetworkSFML
```
